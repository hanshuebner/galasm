...
int
AssemblePldFile(char *file, struct Config *cfg)
{
	UBYTE chr;
			...
			for (pass = 0; pass < 2; pass++)	/* this is a two-pass-assembler */
			{
				...
				/*** GAL16V8, GAL20V8 ***/
					...
					if (gal_type == GAL16V8 || gal_type == GAL20V8)
					{
						...
						{
							if (((OLMC[n].PinType == COMOUT) || (OLMC[n].PinType == TRIOUT) || (OLMC[n].PinType == REGOUT)) && (OLMC[n].Active == ACTIVE_HIGH))
	    ...
	    /*** GAL20RA10 ***/
				...
				{	/* is pin a OLMC pin? */
					if (((gal_type == GAL16V8) &&
						...
						&& (actPin.p_Pin <=
						    22))
					    || ((gal_type == GAL22V10)
						&& (actPin.p_Pin >= 14) && (actPin.p_Pin <= DUMMY_OLMC12)) || ((gal_type == GAL20RA10) && (actPin.p_Pin >= 14) && (actPin.p_Pin <= 23)))
					{
						...
						case SUFFIX_E:
							...
							if (OLMC[n].PinType == REGOUT && (gal_type == GAL16V8 || gal_type == GAL20V8))
							{
				...
				if (pass)
				{
					...
					else
					{
						...
						if (suffix == SUFFIX_E
						    || suffix == SUFFIX_CLK || suffix == SUFFIX_ARST || suffix == SUFFIX_APRST || (gal_type == GAL22V10 && (actOLMC == 10 || actOLMC == 11)))
						{
					...
					if (!IsOR(*actptr) && !IsAND(*actptr) && suffix != SUFFIX_E && suffix != SUFFIX_CLK && suffix != SUFFIX_ARST && suffix != SUFFIX_APRST)
					{
						/* no?, then set unused */
...
void
SetAND(int row, int pinnum, int negation, int gal_type)
{
	int column = 0;
	int numofcol = 0;
...
void
IsPinName(UBYTE * pinnames, int numofpins)
{
	int i, k, n;
	UBYTE *oldactptr;
		...
		if ((n == 2) && !strncmp((char *) oldactptr, "NC", (size_t) 2))
			...
			{	/* examine whole list of pin names */
				...
				if (n == strlen((char *) (pinnames + k * 10 + i)))
					if (!(strncmp((char *) oldactptr, (char *) (pinnames + k * 10 + i), (size_t) n)))	/* yes, then compare these strings */
					{
						actPin.p_Pin = k + 1;
						break;
					}
	...
	if ((fp = fopen(filename, (char *) "w")))
	{
		...
		for (n = 1; n <= num_of_pins; n++)
		{
			...
			if ((gal_type == GAL16V8 && n >= 12 && n <= 19) ||
			    (gal_type == GAL20V8 && n >= 15 && n <= 22) || (gal_type == GAL20RA10 && n >= 14 && n <= 23) || (gal_type == GAL22V10 && n >= 14 && n <= 23))
			{
...
void
WriteRow(FILE * fp, int row, int num_of_col)
{
	int col;
...
void
WriteFuseFile(char *filename, int gal_type)
{
	FILE *fp;
...
void
AsmError(int errornum, int pinnum)
{
	free(fbuff);
